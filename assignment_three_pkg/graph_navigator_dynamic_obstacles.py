...
class GraphNavigator(Node):
    def __init__(self):
        ...
        # Dynamic obstacles
        self.dynamic_obstacles: Optional[np.ndarray] = None
    ...
    def is_free(self, ix: int, iy: int, map_grid: Optional[np.ndarray] = None) -> bool:
        if map_grid is None:
            map_grid = self.map_data
        if ix < 0 or iy < 0 or ix >= self.map_width or iy >= self.map_height:
            return False
        v = int(map_grid[iy, ix])
        if v == -1:
            return False
        return v < self.obstacle_threshold

    def diagonal_allowed(self, cx: int, cy: int, nx: int, ny: int, map_grid: Optional[np.ndarray] = None) -> bool:
        dx = nx - cx
        dy = ny - cy
        if abs(dx) == 1 and abs(dy) == 1:
            return self.is_free(cx + dx, cy, map_grid) and self.is_free(cx, cy + dy, map_grid)
        return True
    ...
    def get_robot_grid_index(self) -> Optional[GridIndex]:
        if not self.map_received:
            return None
        try:
            tf = self.tf_buffer.lookup_transform('map', self.base_frame_id, Time(), timeout=Duration(seconds=0.5))
    ...
    def goal_callback(self, msg: PoseStamped):
        ...
        combined_map = self.get_combined_map()
        self.get_logger().info(f"Planning A* from {start_idx} to {goal_idx}...")
        cell_path = self.a_star(start_idx, goal_idx, combined_map)
        if not cell_path:
    ...
    # ---------------- A* implementation ----------------
    def a_star(self, start: GridIndex, goal: GridIndex, map_grid: Optional[np.ndarray] = None) -> List[GridIndex]:
        grid = map_grid if map_grid is not None else self.get_map()
        if grid is None:
            return []
        ...
        while open_set:
            ...
            for dx, dy in neighbors:
                ...
                neighbor = (nx, ny)
                if not self.is_free(nx, ny, grid):
                    continue
                if not self.diagonal_allowed(current[0], current[1], nx, ny, grid):
                    continue
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        dyn_grid = np.zeros((self.map_height, self.map_width), dtype=np.int8)
        ...
        for i, r in enumerate(msg.ranges):
            ...
            px = r * math.cos(angle)
            py = r * math.sin(angle)
            mx = math.cos(yaw) * px - math.sin(yaw) * py + robot_x
            my = math.sin(yaw) * px + math.cos(yaw) * py + robot_y
            idx = self.world_to_grid(mx, my)
            if idx is None:
                continue
            ix, iy = idx
        ...
        self.dynamic_obstacles = dyn_grid

    def get_combined_map(self) -> Optional[np.ndarray]:
        if not self.map_received:
            return None
        combined = self.map_data.copy()
        if self.dynamic_obstacles is not None:
            combined = np.maximum(combined, self.dynamic_obstacles)
        return combined

    # ---------------- Path following ----------------
    def control_loop(self):
        if not self.path or not self.map_received:
            return
        ...
        combined_map = self.get_combined_map()
        path_indices = [self.world_to_grid(wx, wy) for wx, wy in self.path]
        blocked = any(idx is None or not self.is_free(idx[0], idx[1], combined_map) for idx in path_indices)
        if blocked:
            self.get_logger().info("Dynamic obstacle detected on path, replanning...")
            goal_idx = self.world_to_grid(*self.path[-1])
            start_idx = self.get_robot_grid_index()
